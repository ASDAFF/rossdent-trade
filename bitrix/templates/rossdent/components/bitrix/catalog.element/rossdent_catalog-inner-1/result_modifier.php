...
$arParams['DETAIL_PROPS_ANALOG'] = Array('PROPERTY_PRODUCT_VENDOR');

// Минимальное количество элементов
$minCount = 9;
$outPrice = 'RUB';

// это для того что бы определить какую цену выбирать из базы (Базовая)
...
if ($arr_analogs->SelectedRowsCount() < $minCount) {
...
if (!empty($element['CATALOG_PRICE_1'])) {
	if ($element['CATALOG_CURRENCY_1'] !== $outPrice) {
		$element['PRICE'] = CCurrencyRates::ConvertCurrency($element['CATALOG_PRICE_1'], $element['CATALOG_CURRENCY_1'], $outPrice);
	} else {
		$element['PRICE'] = $element['CATALOG_PRICE_1'];
	}
} else {
	$offers = CCatalogSKU::getOffersList($element['ID'],0,Array(),Array('CATALOG_GROUP_1'));
	$offer = array_pop($offers);
	$offer = array_pop($offer);
	if (!empty($offer['CATALOG_PRICE_1'])) {
		$element['PRICE'] = CCurrencyRates::ConvertCurrency($offer['CATALOG_PRICE_1'], $offer['CATALOG_CURRENCY_1'], $outPrice);
	} else {
		$element['PRICE'] = '';
	}
}
...
if(!$arResult['PRICES']['BASE']['VALUE']){
	...
	$arResult['PRICES']['BASE']['VALUE'] = $convert_price;
	$arResult['PRICES']['BASE']['CURRENCY'] = 'RUB';
}
?>
