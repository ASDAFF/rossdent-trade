...
// News slider in index page for Practic Centre
new Swiper('.pc_news', {
    nextButton: '.news-slider__next',
    prevButton: '.news-slider__prev',
    slidesPerView: 3,
...
(function () {
    ...
    $('.ajax-btn-basket-add').click(function (e) {
        e.preventDefault();
        btn = $(this);
    ...
    $("#catalog-show-more").click(function (e) {
        ...
        $.get(path + "&SECTION_ID=" + catalog.data("section-id") + "&PAGEN_1=" + lastpage,
            {sort: $.urlParam('sort'), order: $.urlParam('order'), limit: $.urlParam('limit')}, function (data) {
                ...
                if ($("#prod"+prod_id).length) {
                    $('html, body').animate({
                        scrollTop: $("#prod" + prod_id).offset().top
                    }, 500);
                }
...
$(function () {
    ...
    country = form.find('[name=PERSONAL_COUNTRY] :selected').text(),
    state = form.find('[name=PERSONAL_STATE]'),
    ...
    btnOrder.click(function () {
        ...
        if (inputValid(name) & inputValid(lastName) & inputValid(email) & inputValid(secondName) & inputValid(phone) & inputValid(state) & inputValid(city) & inputValid(street)) {
            personal = 'ФИО: ' + name.val() + ' ' + lastName.val() + ' ' + secondName.val() + ' Телефон: ' + phone.val() + ' email: ' + email.val();
            adr = ' Адрес доставки: ' + country + ', ' + state.val() + ', ' + city.val() + ', ' + street.val();
...
$(function () {
    $(document).on('click', '.bx-filter-parameters-box a', function (event) {
        var $this = $(this), $next = $this.next();
        if ($next.length) {
            $next.slideToggle().parent().siblings().children('bx-filter-block').filter(':visible').slideToggle();
        event.preventDefault();
}
});
});
...
$(function () {
    $(".bx-filter-parameters-box a.switch-btn").click(function () {
       $(this).toggleClass("open");
   });
});
