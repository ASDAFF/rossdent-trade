...
<article class="lesson-item lesson-item-new"  >
	<div class="lesson-item__date">
		...
		echo $d2->format('j ');
		echo $month[$d2->format('n')]?> <?=$arItem['PROPERTIES']['LESSON_YEAR']['VALUE']?>, <?=$arItem['PROPERTIES']['LESSON_COUNTRY']['VALUE']?>
		...
		echo $d1->format('j ');
		echo $month[$d1->format('n')]?>		
		<?=$arItem['PROPERTIES']['LESSON_UP_DATE']['VALUE']?> <?=$arItem['PROPERTIES']['LESSON_YEAR']['VALUE']?>, <?=$arItem['PROPERTIES']['LESSON_COUNTRY']['VALUE']?>
	...
	<div class="lesson-item__btn">
		<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="btn-green-border" >Подробнее</a>
	</div>
</article>
<?endforeach?>
<?else:?>
