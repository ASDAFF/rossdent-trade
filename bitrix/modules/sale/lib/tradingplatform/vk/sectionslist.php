...
class SectionsList
{
	...
	public function __construct($exportId)
	{
		...
		$this->mappedSections = Map::getMappedSections($exportId);
	}
	
	
	/**
	 * Return list of iblock sections. At the first run saving list in static value - to performance.
	 *
	 * @param bool $tree - is true - list will be converted to tree
	 * @return array
	 * @throws \Bitrix\Main\SystemException
	 */
	private static function getSections($tree = false)
	{
//		in first run - save list in STATIC. Other runs will be faster
		if (!isset(self::$sections))
		{
//			IBLOCK IDS for getting catalog sections
			$iblocks = TradingPlatform\Helper::getIblocksIds();
			$iblocksIds = array();
...
//			get ALL sections for ALL catalog iblocks
			$resSections = \CIBlockSection::GetTreeList(
				array("IBLOCK_ID" => $iblocksIds),
				array("IBLOCK_ID", "IBLOCK_SECTION_ID", "ID", "DEPTH_LEVEL", "NAME", "LEFT_MARGIN", "RIGHT_MARGIN")
			);
			
			while ($currSection = $resSections->Fetch())
			{
//				output format - list or tree separated on iblocks
				self::$sections[$currSection["IBLOCK_ID"]][$currSection["ID"]] = $currSection;
			}
		}
...
//		if not a tree - formatted to list
		...
		{
			$sectionsList = array();
			foreach (self::$sections as $iblock)
				$sectionsList += $iblock;
...
//		if INHERIT and parent section included childs - put section to parent to_album
	 ...
	 * @throws \Bitrix\Main\SystemException
	 */
	public static function getSectionsSelector($checkedSection = NULL)
	{
		$iblockIds = TradingPlatform\Helper::getIblocksIds();
		$sectionsTree = self::getSections(true);
		
		$result = '';
		$result .= '<option value="0">' . Loc::getMessage("SALE_CATALOG_VK_MAIN_ALBUM") . '</option>';
		foreach ($iblockIds as $iblock)
		{
//			todo: why strtoupper d	ont work ? encode problem ?
